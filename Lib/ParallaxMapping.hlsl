...
float2 ParallaxOcclusionOffset(float heightScale,float3 viewDirTS,float2 uv,TEXTURE2D_PARAM(depthTex,depthTexSampler),half2 layerRange=half2(8,30)){
    ...
    float curDepth = SAMPLE_TEXTURE2D(depthTex,depthTexSampler,curUV).x;
    UNITY_LOOP while(curLayerDepth < curDepth){
        curUV -= deltaUV;
        curDepth = SAMPLE_TEXTURE2D(depthTex,depthTexSampler,curUV).x;
    ...
    float beforeDepth = SAMPLE_TEXTURE2D(depthTex,depthTexSampler,prevUV).x - curLayerDepth + layerDepth;
